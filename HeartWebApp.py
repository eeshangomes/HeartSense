...
def app():
    ...
    unsafe_allow_html=True
        ...
        #     rbp=st.number_input(label="Systolic Blood Pressure",value=120)
        ...
        if st.button('Click To Check Prediction'):
            ...
            # if medical_tick==True:
            #     if diagnosis == 'You are at a high risk of suffering from heart disease' and medical_diagnosis == 'According to medical data, you are at a risk of suffering from heart disease':
            #         final_diagnosis = 'You are at a high risk of suffering from heart disease, according to your medical and non-medical parameters. Please consult a heart specialist immediately'
            #     elif diagnosis == 'You are not at a risk of suffering from heart disease' and medical_diagnosis == 'According to medical data, you are at a risk of suffering from heart disease':
            #         final_diagnosis = 'Your non-medical parameters seem alright, but according to your medical parameters, you are at a high risk of suffering from heart diseases. Please consult a heart specialist immediately'
            #     elif diagnosis == 'You are not at a risk of suffering from heart disease' and medical_diagnosis == 'According to medical data, you are NOT at a risk of suffering from heart disease':
            #         final_diagnosis = 'You are not at risk of suffering from heart disease'
            #     elif diagnosis == 'You are at a high risk of suffering from heart disease' and medical_diagnosis == 'According to medical data, you are NOT at a risk of suffering from heart disease':
            #         final_diagnosis = 'Your medical parameters seem alright, but according to your non-medical parameters, you are at a high risk of suffering from heart disease. Please consult a heart specialist immediately'
            #     prediction_data = {
            #     'input_parameters': [bmi,smoking,alcohol,stroke,physicalhealth,mental,diffwalk,sex,realage,race,diabetic,physactivity,genhealth,sleeptime,asthma,kidney,skin,rbp,chol,fbs],  # Store input parameters as an array
            #     'heart_disease_diagnosis': final_diagnosis
            #     #'medical_condition_diagnosis': medical_condition_diagnosis
            # }
            ...
            prediction_data = {
                    'input_parameters': [bmi,smoking,alcohol,stroke,physicalhealth,mental,diffwalk,sex,agecat,race,diabetic,physactivity,genhealth,sleeptime,asthma,kidney,skin],  # Store input parameters as an array
                    'heart_disease_diagnosis': final_diagnosis
                    #'medical_condition_diagnosis': medical_condition_diagnosis
                }
            #st.success(diagnosis)
            ...
            doc_ref=db.collection('Past Records').document(st.session_state.username).collection('Predictions').document(timestamp)
        ...
        #     timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        #     prediction_data = {
        #         'input_parameters': [bmi, smoking, alcohol, stroke, physicalhealth, mental, diffwalk, sex, realage, race, diabetic, physactivity, genhealth, sleeptime, asthma, kidney, skin, rbp, chol, fbs],
        #         'heart_disease_diagnosis': final_diagnosis
        #     }
        #     doc_ref = db.collection('Past Records').document(st.session_state.username).collection('Predictions').document(timestamp)
        #     doc_ref.set(prediction_data)
